...
def init_db():
    ...
    if 'category' not in columns:
        cursor.execute("ALTER TABLE passwords ADD COLUMN category TEXT DEFAULT 'Без категории'")
    if 'created_at' not in columns:
        cursor.execute("ALTER TABLE passwords ADD COLUMN created_at TEXT")
    if 'updated_at' not in columns:
        cursor.execute("ALTER TABLE passwords ADD COLUMN updated_at TEXT")
...
def add_password_entry(user_id, service, login, password, url, category="Без категории"):
    ...
    cursor.execute("""INSERT INTO passwords 
                      (user_id, service, login, password_enc, url, category, created_at, updated_at) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                   (user_id, service, login, enc_pwd, url, category, now, now))
    
    log_action(user_id, f"Добавлен пароль для {service} (категория: {category})")
    conn.commit()
    conn.close()
...
def get_user_passwords(user_id):
    ...
    if 'category' in columns and 'created_at' in columns and 'updated_at' in columns:
        cursor.execute("""SELECT id, service, login, password_enc, url, category, created_at, updated_at 
                          FROM passwords WHERE user_id = ? ORDER BY service""", (user_id,))
        ...
        for row in rows:
            ...
            result.append({
                ...
                "password": dec_pwd, "url": row[4], "category": row[5] if len(row) > 5 else "Без категории",
        ...
        for row in rows:
            ...
            result.append({
                "id": row[0], "service": row[1], "login": row[2], 
                "password": dec_pwd, "url": row[4], "category": "Без категории",
                "created_at": "", "updated_at": ""
            })
    return result

def update_password_entry(entry_id, service, login, password, url, category):
    ...
    cursor.execute("""UPDATE passwords 
                      SET service=?, login=?, password_enc=?, url=?, category=?, updated_at=?
                      WHERE id=?""",
                   (service, login, enc_pwd, url, category, now, entry_id))
...
def get_categories(user_id):
    ...
    if 'category' in columns:
        cursor.execute("SELECT DISTINCT category FROM passwords WHERE user_id = ? ORDER BY category", (user_id,))
        rows = cursor.fetchall()
        conn.close()
...
def get_passwords_by_category(user_id, category):
    ...
    if 'category' in columns and 'created_at' in columns and 'updated_at' in columns:
        cursor.execute("""SELECT id, service, login, password_enc, url, category, created_at, updated_at 
                          FROM passwords WHERE user_id = ? AND category = ? ORDER BY service""", (user_id, category))
        rows = cursor.fetchall()
        conn.close()
        ...
        for row in rows:
            ...
            result.append({
                "id": row[0], "service": row[1], "login": row[2], 
                "password": dec_pwd, "url": row[4], "category": row[5],
